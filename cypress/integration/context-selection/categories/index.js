...
Given("a data set, org unit & period have been selected and the current date is outside the range of all of the option's validity dates of one category", () => {
    cy.intercept(
        ...
        { fixture: 'context-selection/cat-combo-with-all-options-of-one-cat-between-20200101-20201231.json' }
    )
...
Given("a data set, org unit & period have been selected and the current date is outside the range of one of the category option's validity dates", () => {
    cy.intercept(
        'GET',
        new RegExp('dataSets/lyLU2wR22tC[?]fields=categoryCombo'),
        { fixture: 'context-selection/cat-combo-with-option-between-20200101-20201231.json' }
    )
...
Given("a data set, org unit & period have been selected and the data set's category combination is the default one", () => {
    cy.visit('/#/?dataSetId=BfMAe6Itzgt&orgUnitId=ImspTQPwCqd&periodId=202212')
    cy.get('[data-test="data-set-selector"]').should('exist')
})
...
Given('a data set, org unit, period and some but not all options have been selected and there are some categories connected to the data set', () => {
    cy.visit('/#/?dataSetId=lyLU2wR22tC&orgUnitId=ImspTQPwCqd&periodId=202212&attributeOptionComboSelection=LFsZ8v5v7rq-CW81uF03hvV')
    cy.get('[data-test="data-set-selector"]').should('exist')
})

When('the user opens the dropdown for the category of that option', () => {
    cy.get('[data-test="attribute-option-combo-selector"]').click()
    cy.contains('[data-test="dhis2-uiwidgets-singleselectfield"]', 'Implementing Partner')
...
Then('the default options should be selected automatically', () => {
    cy.get('[data-test="dhis2-ui-selectorbar"]').contains('Default attribute combo')
})

Then('the selector indicates that the default category and default option has been selected', () => {})
...
Then('the selector should show that zero items have been selected', () => {
    cy.get('[data-test="dhis2-ui-selectorbar"]').contains('0 selections').should('exist')
})

Then('the selector should show that some items have been selected', () => {
    cy.get('[data-test="dhis2-ui-selectorbar"]').contains('1 selections').should('exist')
})
