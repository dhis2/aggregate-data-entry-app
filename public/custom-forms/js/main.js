...
dhis2.leftBar.setLinks = function (showLeftBarLink, showExtendMenuLink) {
    ...
    $('#extendMainMenuLink').css(
        'display',
        showExtendMenuLink ? 'inline' : 'none'
    )
}
...
dhis2.leftBar.extendAnimated = function () {
    ...
    $('#leftBar, #orgUnitTree')
        .show()
        .animate({ direction: 'left', width: '+=150px', duration: 20 })
    $('#mainPage').animate({
        direction: 'left',
        marginLeft: '+=150px',
        duration: 20,
...
dhis2.leftBar.retract = function () {
    ...
    $('#leftBar, #orgUnitTree')
        .show()
        .animate({ direction: 'right', width: '-=150px', duration: 20 })
    $('#mainPage').animate({
        direction: 'right',
        marginLeft: '-=150px',
        duration: 20,
    })
    $('#hideMainMenuLink').attr(
        'href',
        'javascript:javascript:dhis2.leftBar.hideAnimated()'
    )
}
...
dhis2.leftBar.openHelpForm = function (id) {
    window.open(
        '../dhis-web-commons/help/viewDynamicHelp.action?id=' + id,
        'Help',
